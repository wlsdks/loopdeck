...
`looprelay` must stay local-first and provider-neutral:
...
- LLM rewrite and judge behavior is opt-in and agent-mediated. `looprelay`
...
The risky design would be for `looprelay` to silently spawn provider CLIs or
...
3. `looprelay` returns only bounded metadata and, when requested, locally
...
5. The agent calls `record_agent_rewrite` with the improved draft after user
   ...
   suggestions.
6. `looprelay` stores only redacted rewrite drafts or judgment metadata.
...
- Do not ask users to paste provider tokens into `looprelay`.
