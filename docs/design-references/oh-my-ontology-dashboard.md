...
이 문서는 사용자가 지정한 `<workstation>/side-project/oh-my-ontology` 디자인을 `looprelay`에 이식하기 위한 기준이다.
...
- `looprelay` Dashboard는 ontology map 대신 prompt habit data를 주인공으로 보여준다.
