...
이 문서는 `<workstation>/ai/awesome-design-md`와 `getdesign linear.app` reference를 `looprelay` Dashboard에 맞게 적용한 내부 기준이다. Linear를 복제하지 않고, looprelay의 local-first developer tool 정체성에 맞게 번역해서 쓴다.
...
- accent color는 한 가지 역할에 집중한다. `looprelay`에서는 teal을 유지하고, dark command surface 내부에서만 lavender-blue reference를 약하게 빌려 쓴다.
