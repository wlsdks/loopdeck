...
This document defines the technical design for LoopRelay, a local-first coding-agent continuity and evidence layer for Codex, Claude Code, and long-running coding-agent work. Loop features are loop-aware continuation for the next prompt across sessions, worktrees, branches, and compact boundaries. The current npm package and CLI command remain `looprelay` during the compatibility window.
...
1. User installs hooks through `looprelay setup` or `install-hook`.
...
1. User registers `looprelay mcp` with a local MCP client such as Claude Code
   or Codex.
2. The client launches the command as a stdio subprocess.
3. The MCP server exposes `get_looprelay_status`, `coach_prompt`,
...
4. `get_looprelay_status` checks whether local storage is initialized,
...
9. `prepare_agent_rewrite` returns one locally redacted prompt body, local score
   ...
   Claude Code, Codex, or Gemini CLI session. `looprelay` does not call a
...
13. `prepare_agent_judge_batch` returns a bounded set of locally redacted
    ...
    session to judge. `looprelay` does not call a provider or route
...
- no hidden external LLM calls are made by `looprelay`
...
LoopRelay creates copy-based improvement drafts. Drafts:
