...
**Goal:** Reposition `looprelay` into a local-first agent loop memory and meta-prompting workbench that is tightly integrated with Codex and Claude Code.

**Current product name:** LoopRelay

**Superseded naming note:** This spec originally recommended `LoopRelay`.
That name is now rejected as the primary product name because it makes the
product sound loop-engineering-first. Use `LoopRelay` for product-facing copy;
keep `looprelay` as the runtime compatibility ID; keep `looprelay` only as a
legacy CLI alias or historical planning term.

**Repository target:** `wlsdks/looprelay`

**Status:** Historical architecture baseline with active implementation notes.
Use `docs/superpowers/specs/2026-07-05-looprelay-repositioning-design.md` and
`docs/LOOPRELAY.md` as the current naming contract. Do not rename packages,
remotes, public commands, slash namespaces, or MCP server names from this
document alone.

---

## 1. Why This Is Bigger Than LoopRelaying
...
> LoopRelay is a local-first workbench for recording, understanding, and improving coding-agent loops across Codex, Claude Code, worktrees, and long-running sessions.
...
The current name `looprelay` is accurate for the old scope but too narrow for this. It implies prompt linting. The target product is closer to loop observability, loop memory, agent handoff, and meta-prompt synthesis.
...
- package name: `looprelay`
- current GitHub repo metadata via `gh repo view`: `wlsdks/looprelay`
- local git remote points at `https://github.com/wlsdks/looprelay.git`
...
- The first version of this spec defined direction and a first slice, but did not explicitly decide which existing features stay unchanged, which become LoopRelay features, which should be deprecated, and which are new.
...
| Agent wrappers `lr-claude` / `lr-codex` | Experimental initial prompt wrappers                                                                        | **Do not expand now**               | Hooks and MCP are tighter official surfaces. Wrappers do not cover later interactive turns.                                |
...
| Repo/package rename                     | Partially renamed repo metadata, package still `looprelay`                                               | **Plan, do not execute in Slice 1** | Branding is important but should not block proving the loop model.                                                         |
...
- npm availability check returned 404 for `looprelay` at the time of investigation.
...
- `prompt-loop`: better than looprelay, but still too prompt-centric.
- `looprelay-ai`: useful fallback if `looprelay` is unavailable on GitHub/npm, but less clean.
...
1. Rename GitHub repository from `wlsdks/looprelay` to `wlsdks/looprelay`. (**implemented**)
2. Update local `origin` from stale looprelay/looprelay URLs to the new repo URL. (**implemented**)
3. Update README and docs to say `LoopRelay` as product name while preserving `looprelay` as the current CLI/package during a compatibility window. (**implemented**)
4. Add `looprelay` CLI/package rename as a separate migration plan because it touches npm, plugin commands, docs, binary names, package files, and tests.
...
- Automations only as a user-facing scheduling surface; LoopRelay should not secretly create Codex automations without explicit setup.

Required Codex-facing LoopRelay capabilities:

- `looprelay status`: show whether hooks, MCP, server, and capture are active for the current repo.
- `looprelay loop collect`: collect one loop snapshot for the current cwd/session.
- MCP `get_looprelay_status`: expose safe readiness and next actions.
...
- `looprelay setup --profile coach --register-mcp --open-web` still works during the LoopRelay transition.
...
Future Claude-facing LoopRelay namespace target:

- `/looprelay:setup`
- `/looprelay:status`
- `/looprelay:loop`
- `/looprelay:brief`
- `/looprelay:review-loop`
- `/looprelay:open`

Current Claude Code slash commands remain `/looprelay:*`. The `/looprelay:*`
...
- The Claude plugin can expose `/looprelay:*` commands later, but existing `/looprelay:*` commands must continue during migration.
...
LoopRelay should not compete with LangGraph, ADK, Temporal, or hosted agent runtimes. Those systems own durable execution, graph orchestration, and generalized tool dispatch.

LoopRelay's lane is narrower:
...
- `docs/LOOPRELAY.md`: product thesis, loop model, user workflows. (**implemented as the product contract**)
...
| ------------------------------ | ------------------------------------------------------------------ | ------------------------------- | ----------- |
| `docs/LOOPRELAY.md`             | Product thesis, loop model, user workflows, feature portfolio      | Brand migration or web Loops UI | implemented |
...
LoopRelay can improve itself only through an approval-based loop.
...
- CLI `looprelay loop status`
- CLI `looprelay loop collect`
- CLI `looprelay loop brief`
...
- existing `looprelay` MCP tools still pass.
...
- loop status and brief awareness (**implemented for CLI `loop status`, CLI
  `loop brief`, MCP `get_looprelay_status`, and MCP `prepare_loop_brief`; it
...
- `looprelay loop collect --source service` for cron or LaunchAgent one-shot
  collection (**implemented**)
- output that labels service-origin snapshots without prompt bodies or raw paths
  (**implemented**)
- opt-in macOS LaunchAgent preview/install via `looprelay loop schedule
install` (**implemented; dry-run supported**)
- explicit scheduler lifecycle commands via `looprelay loop schedule status`
  and `looprelay loop schedule uninstall` (**implemented; plist-only status
  and removal**)
...
- CLI `looprelay loop memory-candidate` for a local, copy-reviewable memory
...
- local SQLite `loop_memories` records for user-approved memory candidates
  (**implemented**)
- CLI `looprelay loop memory-approve` to record the latest eligible
...
- pure proposal generator that converts the latest approved LoopRelay memory
  into a reviewable unified diff for `AGENTS.md` or `CLAUDE.md`
  (**implemented**)
- CLI `looprelay loop instruction-patch --target-file AGENTS.md` for local
...
- pure apply function that appends the latest approved LoopRelay memory to
  `AGENTS.md` or `CLAUDE.md` only when explicit confirmation is present
  (**implemented**)
- CLI `looprelay loop instruction-apply --target-file AGENTS.md
...
- product name in README/docs: LoopRelay (**implemented**)
...
- explicit compatibility statement that npm package name, CLI binary, and
  plugin command ids remain `looprelay` during the transition
  (**implemented**)

Do not add:

- `looprelay` CLI/package alias in the same slice
- npm package rename
...
- model-level tests proving no prompt bodies, compact summaries, custom
  ...
  returned
- CLI `looprelay loop status` powered by the shared model
- MCP `get_looprelay_status` powered by the shared model, with MCP-only
...
- `createLoopBrief` optional approved memory input for local, raw-free memory
  statements
- CLI `looprelay loop brief` inclusion of latest approved loop memories
...
- CLI `looprelay loop brief` project-scoped approved memory lookup using
...
- shared `LoopRelayStatus.project_memory` summary with only
  `approved_count` and `included_in_brief`
- CLI `looprelay loop status` text and JSON showing the latest snapshot
  project's approved memory count
- MCP `get_looprelay_status` result and schema exposing the same count
...
- `package.json#bin.looprelay` pointing to the same compiled CLI entrypoint as
  `looprelay`
- packaging tests proving `looprelay` remains the package name and primary
  compatibility command while `looprelay` is available as a product-name CLI
  alias
- README and package contents docs explaining that existing scripts and plugin
  commands should keep using `looprelay`, while new manual terminal workflows
  may use `looprelay`

Do not add:

- npm package rename
- removal of `looprelay` bin
- plugin command id rename
- hook marker rename
...
- shared `LoopRelayStatus.memory_candidate` summary with only `eligible`,
  `reason`, and `next_action`
- CLI `looprelay loop status` text and JSON showing whether the latest loop
  can be approved into local project memory
- MCP `get_looprelay_status` result and schema exposing the same raw-free
  candidate summary
...
- Claude Code command docs that check for either `looprelay` or `looprelay`
  when running manual CLI fallbacks
- setup/status/coach command docs that show `looprelay` as the product-name CLI
  alias while preserving existing `looprelay` examples
- Codex plugin default prompt that advertises the `looprelay` CLI alias for
  manual terminal workflows
- README and PLUGINS documentation clarifying that Claude Code slash commands
  remain under `/looprelay:*` during the migration
- packaging tests proving this compatibility guidance is present

Do not add:

- `/looprelay:*` slash command namespace before a dedicated plugin rename plan
- removal of `/looprelay:*` commands
- plugin id rename
- hook marker rename
- MCP server name rename
...
- CLI `looprelay loop memory-approve --json` result with structured
  `next_actions`
...
- web API `GET /api/v1/loops/instruction-patch?target_file=AGENTS.md` for the
  latest approved LoopRelay memory
...
- `docs/superpowers/plans/2026-07-04-looprelay-plugin-rename-plan.md` as the
...
- packaging tests proving the plan exists and locks the current
  `looprelay` package, primary CLI, plugin id, `/looprelay:*` slash
...
- `/looprelay:*` slash commands
- plugin id rename
- npm package rename
- removal of the `looprelay` CLI
- hook marker rename
- MCP server name rename
...
- shared `LoopRelayStatus.activity` summary derived from existing loop snapshots
...
- `LoopRelayStatus.activity.worktrees` detail derived only from existing local
  loop snapshots
...
- MCP `get_looprelay_status`, `/api/v1/loops`, web API type, and Loops summary
...
- dedicated `docs/superpowers/plans/2026-07-04-looprelay-plugin-rename-issue-slices.md`
  that decomposes the rename plan into independently shippable issues
- compatibility invariants that preserve `looprelay` package, binary,
...
- dedicated `docs/superpowers/plans/2026-07-04-looprelay-runtime-id-inventory.json`
...
- README, README.ko, and PLUGINS documentation that names `/looprelay:*` as a
  future alias-only slash namespace
- packaging test coverage that proves the docs mention both `/looprelay:*`
  and `/looprelay:*` while package, plugin ids, and Claude Code command files
  remain unchanged
- compatibility wording that `/looprelay:*` must not become the only namespace
  and is not shipped as command files in this slice

Do not add:

- `/looprelay:*` command files
- Claude Code plugin manifest command entries for looprelay aliases
- package, plugin id, hook command, or MCP server-name changes
...
- R3 decision artifact documenting that current Claude Code command namespace is
  tied to plugin identity, so `commands/looprelay-*.md` would not create
  `/looprelay:*`
- packaging test coverage that keeps `/looprelay:*` required and prevents
  accidental looprelay command file/manifest additions
...
- Codex plugin interface copy and default prompts that use LoopRelay as the
  product-facing name
...
- packaging test coverage that locks Codex plugin id, install path, skill path,
  and hook command compatibility while display copy says LoopRelay

Do not add:

- Codex plugin id rename
- `plugins/looprelay` install path rename
...
- hook binary smoke script that runs built `looprelay` and `looprelay`
...
- `looprelay hook ...` as the primary installed hook command
...
- R6 decision artifact documenting that `looprelay` remains the canonical MCP
  server name during the LoopRelay compatibility window
...
- packaging test coverage that keeps README, README.ko, PLUGINS, slash setup
  docs, and web MCP snippets on `looprelay mcp`

Do not add:

- `mcp add looprelay` examples
- MCP server-name rename
...
- R7 readiness artifact with alias-only, deprecation, and breaking release note
  templates
- support guidance for saved `/looprelay:*` slash command snippets
- minimum evidence gates before marking any `looprelay` runtime id deprecated
...
- deprecation banners
- removal of `/looprelay:*`, plugin ids, hook commands, package names, or MCP
  server names
...
- shared `LoopRelayStatus.activity.command_center` summary for review-before-merge
  situations
...
- MCP `get_looprelay_status` output schema coverage for the command center
...
- MCP `get_looprelay_status` schema coverage for `continuation_command`
...
- MCP `get_looprelay_status` output schema coverage for the new fields
...
- `review_packet` metadata on `LoopRelayStatus.activity.command_center`, derived
...
- MCP `get_looprelay_status` output schema coverage for the packet contract
...
- MCP `get_looprelay_status` output schema coverage for checklist entries
...
- `looprelay loop decision record --worktree <name> --decision <merge|continue|defer> --reason <text>`
- `looprelay loop decision list`
...
- `LoopRelayStatus.activity.recent_decisions`, limited to the latest three local
  decisions
- CLI `loop status` lines for recent decisions
- MCP `get_looprelay_status` schema/runtime coverage for the read-only field
...
- merge decision writes outside `looprelay loop decision record`
...
- MCP `get_looprelay_status` schema/runtime coverage for the advisory
...
- top-level selected worktree detail `selected_brief_action` when a selected
  ...
  - command: `looprelay loop brief --worktree ...` plus optional `--session`
    and `--branch` filters
  - writes_files: `false`
...
- The provenance must not imply that LoopRelay read git state, checked branch
...
- This is necessary because Codex and Claude Code are primary runtime surfaces
  for LoopRelay, and the web detail panel should connect copy-ready loop memory
...
- This is needed because LoopRelay is not just a prompt archive; it is a
...
- This is needed because LoopRelay is becoming an agent loop memory workbench,
...
- Selected worktree detail should show the privacy boundary for the
  source-of-truth model. It should say that LoopRelay stores local loop metadata
...
- This is needed because paste destination and handoff checklist explain where
  the brief goes, but the UI should also make clear that LoopRelay does not
...
- This is needed because the handoff checklist says to collect the next
  snapshot, but does not explicitly prevent misreading LoopRelay as a background
...
- This is needed because the ordering note tells the operator to review safety
  guidance first, but should not imply LoopRelay tracks a completed checklist or
...
- Selected worktree detail should state that retrying the selected brief copy is
  a manual operator action and that LoopRelay does not automatically retry
...
- This is needed because target-agent checking should not be mistaken for
  LoopRelay inspecting agent UI state, reading target contents, or validating
...
- This is needed because final submission guidance should not be mistaken for
  LoopRelay pressing Enter, clicking submit, detecting submission success, or
...
- Selected worktree detail should state that agent response and submission
  result stay outside LoopRelay until the next explicit loop snapshot.
...
- Selected worktree detail should state that retry attempt and retry outcome
  state stay outside LoopRelay until the next explicit loop snapshot.
...
- Selected worktree detail should state that freshness result state stays
  outside LoopRelay until the next explicit loop snapshot.
...
- This is needed because freshness uncertainty can affect whether a continuation
  handoff is ready to merge, but LoopRelay should not approve merges, verify
...
- Selected worktree detail should state that post-memory-approval collection
  result state stays outside LoopRelay until the next explicit loop snapshot is
  recorded.
- This is needed because the operator may collect after memory approval, but
  LoopRelay should not infer, persist, or synchronize that collection result from
...
- Selected worktree detail should state that post-memory-approval retry outcome
  stays outside LoopRelay until the next explicit loop snapshot.
- This is needed because the retry boundary would otherwise imply that LoopRelay
...
- This is needed because post-approval retry outcome can be stale after another
  loop turn. LoopRelay should present the review boundary without reading git
...
- Selected worktree detail should state that post-memory-approval retry
  freshness result stays outside LoopRelay until the next explicit loop snapshot.
- This is needed because the freshness boundary should not imply that LoopRelay
...
- This is needed because renewed memory approval can be mistaken for a trigger
  that starts collection automatically; LoopRelay should keep the collection
...
- Selected worktree detail should state that renewed-memory-approval collection
  result state stays outside LoopRelay until the next explicit loop snapshot.
- This is needed because the renewed approval collection reminder should not
  imply that LoopRelay detects, stores, or synchronizes the result of that
  collection attempt.
...
- This is needed because the operator can manually hand off a continuation
  brief after renewed memory approval; LoopRelay should cue the freshness review
...
- This is needed because paste is the point where the local continuation brief
  crosses into an agent request box; LoopRelay should cue the freshness review
...
- This is needed because submission is the last manual gate before the agent
  acts on the pasted brief; LoopRelay should cue the freshness review without
...
- This is needed because submission can make freshness and agent response state
  diverge from the approved memory; LoopRelay should cue explicit local snapshot
...
- This is needed because the post-submit freshness advisory asks the operator
  to collect a new explicit loop snapshot, but LoopRelay should not imply that it
...
- This is needed because the post-submit collection result boundary can leave
  uncertainty, but LoopRelay should not imply that it retries collection commands
...
- Selected worktree detail should state that post-submit retry attempt and
  outcome stay outside LoopRelay until the next explicit loop snapshot.
- This is needed because the post-submit collection retry boundary should not
  imply that LoopRelay detects, stores, or syncs retry success/failure state
...
- This is needed because the post-submit retry outcome boundary should not
  imply that LoopRelay verifies evidence freshness from git status, transcripts,
...
- This is needed because the post-submit retry evidence freshness boundary
  should not imply that LoopRelay detects, stores, or syncs freshness result
...
- This is needed because renewed memory approval must remain separate from
  post-submit retry freshness uncertainty review. LoopRelay should not imply that
...
- This is needed because renewed memory approval after post-submit retry can
  change the next local memory input, but LoopRelay must not start collection or
...
- Selected worktree detail should state that post-submit retry
  renewed-memory-approval collection result state stays outside LoopRelay until
  the next explicit loop snapshot.
- This is needed because the collection reminder can otherwise imply LoopRelay
  tracks whether that collection completed. LoopRelay should not detect, store,
...
- This is needed because the non-persistence note prevents hidden state
  ...
  when they cannot tell whether that collection result is usable. LoopRelay
...
- This is needed because the collection uncertainty reminder tells the operator
  ...
  LoopRelay should not approve merges or verify freshness before merge.
...
- This is needed because continuation handoff can move stale or uncertain work
  into Codex or Claude Code even when merge remains separately gated. LoopRelay
...
- This is needed because paste is the last step before the copied continuation
  brief leaves LoopRelay's local workspace and enters an agent request box.
  LoopRelay should not approve paste targets or verify freshness before paste.
...
- This is needed because submission is the point where the pasted continuation
  brief becomes an agent action. LoopRelay should not approve submissions,
...
- This is needed because after the operator submits in Codex or Claude Code,
  LoopRelay should not monitor submitted state, agent responses, or freshness
...
- Selected worktree detail should state that post-submit collection result after
  post-submit retry renewed-memory-approval stays outside LoopRelay until the
  next explicit loop snapshot.
- This is needed because the post-submit freshness advisory could otherwise be
  misread as completion tracking. LoopRelay should not detect, store, or sync
...
- This is needed because the non-persistence note only states where result
  ...
  result uncertainty remains, without LoopRelay verifying that result or starting
...
- This is needed because collection uncertainty can affect merge readiness, but
  LoopRelay must not approve merges or verify freshness before merge. Merge
...
- This is needed because collection uncertainty can affect the next handoff,
  but LoopRelay must not approve handoffs or verify freshness before handoff.
...
- This is needed because collection uncertainty can affect whether the copied
  continuation brief is still suitable for the target agent, but LoopRelay must
...
- This is needed because collection uncertainty can affect whether the pasted
  continuation brief is still ready to submit, but LoopRelay must not approve
...
- This is needed because freshness uncertainty after submit must be resolved by
  explicit local snapshot collection, but LoopRelay must not monitor submitted
...
- This is needed because the post-submit collection post-submit freshness
  ...
  freshness is uncertain, but LoopRelay must not detect, store, sync, or infer
...
- Keep the public CLI command as `looprelay` through Slice 1. `looprelay` can become an alias or package rename only in a brand migration slice.
...
- What exact product/package migration date should move from `looprelay` to `looprelay`?
- Should loop snapshots get a numeric "loop quality" score, or only structured outcome status and evidence?

## 12. Decision

Proceed with LoopRelay as the product direction, but treat the planning document as the primary artifact until the go/no-go gate is satisfied.
...
The plan must use TDD and preserve the existing `looprelay` command until a separate brand migration plan is approved.
