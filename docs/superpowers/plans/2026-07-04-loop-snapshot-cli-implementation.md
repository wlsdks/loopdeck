...
**Goal:** Build the first LoopRelay slice by adding a privacy-safe loop snapshot model plus `looprelay loop collect` and `looprelay loop brief`.

**Architecture:** Keep the existing `looprelay` CLI/package name. Add a small `src/loop/` domain module that derives loop snapshots and continuation briefs from existing prompt summaries, then persist snapshots through the SQLite storage boundary. CLI code should orchestrate storage and formatting only; prompt analysis and privacy decisions stay in `src/loop` and `src/storage`.
...
- the public command remains `looprelay`
...
function createTempDir(): string {
  const dir = join(tmpdir(), `looprelay-loop-${crypto.randomUUID()}`);
...
export function loopBriefForCli(options: LoopCliOptions = {}): string {
  return withStorage(options.dataDir, (storage) => {
    ...
    if (!snapshot) {
      throw new UserError(
        "No loop snapshot found. Run `looprelay loop collect` first.",
      );
    }
...
- [x] `looprelay loop collect` and `looprelay loop brief`
...
```bash
gh pr edit 280 --repo wlsdks/looprelay --body-file /tmp/looprelay-pr-body.md
```
...
```md
## Summary
- Adds the LoopRelay product direction and agent loop memory design.
- Implements the first CLI/storage/domain slice for privacy-safe loop snapshots.
- Adds `looprelay loop collect` and `looprelay loop brief` without renaming the package, CLI, hooks, MCP tools, or web UI.
...
- `looprelay` command remains the public command.
