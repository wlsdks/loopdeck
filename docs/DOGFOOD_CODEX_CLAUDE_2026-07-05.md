...
This document records the repeatable local dogfood evidence for the LoopRelay
...
| --- | --- | --- |
| `corepack pnpm smoke:agent-setup` | PASS | Ran `looprelay setup --profile coach --register-mcp` in dry-run and real isolated setup modes, initialized storage for doctor checks, then ran `looprelay doctor claude-code` and `looprelay doctor codex` against fake provider binaries. |
| `corepack pnpm dogfood:first-coach-loop` | PASS | Built the package, initialized an isolated LoopRelay archive, started a local server, captured a Codex prompt through the hook, then ran `coach --json`, `loop collect --json`, and `loop brief --json`. |
| `corepack pnpm smoke:hooks` | PASS | Verified `looprelay` and `looprelay` compatibility hook status plus Claude Code and Codex fail-open hook behavior. |
...
- Codex setup readiness is covered by `looprelay doctor codex` inside
  `smoke:agent-setup`.
- Claude Code setup readiness is covered by `looprelay doctor claude-code`
  inside `smoke:agent-setup`.
- Hook behavior is covered for both tools by `smoke:hooks`, including fail-open
  behavior when no local LoopRelay server is available.
...
- Agent-readable `score_prompt effectiveness evidence` is covered by
  ...
  UI or shelling out to `looprelay show --json`.
