# LoopRelay Codex And Claude Local Integration Evidence 2026-07-06

This document records current non-operator Codex and Claude Code integration
evidence for the LoopRelay 9.5 quality bar.
...
| `corepack pnpm smoke:hooks` | PASS | Verifies `looprelay` hook status, Claude Code fail-open behavior, Codex fail-open behavior, and `looprelay` compatibility hook status. |
...
| `LOOPRELAY_NATIVE_DIALOG_APPROVED=1 corepack pnpm dogfood:mcp-native-dialog-approved` | PASS | After explicit operator approval, opens the native macOS dialog fallback through MCP and completes with `interaction_status: "answered"` plus `approved native dialog dogfood passed`. |
