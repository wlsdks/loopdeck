...
`initializeLoopRelay` creates two subdirectories under the looprelay
...
A grep across `src/` confirms no code path beyond `initializeLoopRelay`
and `getLoopRelayPaths` references either directory. They are created,
...
- The capture path writes the entry to `prompts/<date>/<id>.md` as today
  ...
  scoring.
- `looprelay doctor` would surface a non-zero quarantine count as a
  next step.
...
- When `127.0.0.1:17373` is unreachable, the Claude Code / Codex hook
  ...
  losing it.
- `looprelay server` (or a small drainer in `src/hooks/`) replays
  the spool on next startup and ingests the entries in order.

When this lands, the change set is:

- A `spool` write path in the hook fail-open branch (`src/hooks/`).
- A drain hook in the server boot path or in `looprelay service start`.
- A `spool size` line in `looprelay doctor`.
...
- Continue creating both directories during `initializeLoopRelay` and
  exporting the paths from `getLoopRelayPaths` and `config.json`.
