...
Phase 2 expands `looprelay` from a safe local prompt archive into a local-first loop memory and improvement workspace.
...
After Phase 2, `looprelay` may expose explicit MCP handoff tools that return
...
- beta feedback loop for LoopRelay quality
