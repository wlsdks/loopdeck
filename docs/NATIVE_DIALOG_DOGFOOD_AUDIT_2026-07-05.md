...
approved local run of `LOOPRELAY_NATIVE_DIALOG_APPROVED=1 corepack pnpm
...
- Starts the real `looprelay mcp` server over stdio.
- Forces `LOOPRELAY_NATIVE_DIALOG=0`.
...
2. Otherwise return `clarifying_questions` and let the agent ask through its
   native ask UI.
3. Use `allow_native_dialog: true` or `LOOPRELAY_NATIVE_DIALOG=1` only for an
...
`LOOPRELAY_NATIVE_DIALOG_APPROVED=1` is set. Do not add it to automated CI or
scheduled checks.
