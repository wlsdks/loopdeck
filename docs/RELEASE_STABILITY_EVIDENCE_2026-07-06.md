...
| `corepack pnpm smoke:package-install` | PASS   | Built server and web assets, packed `promptlane-1.0.0.tgz`, installed it into an isolated npm prefix, verified the shipped `promptlane`, `pl-claude`, and `pl-codex` bin help paths, and checked `promptlane start --open-web --json` returns the first-success setup, coach, and doctor commands.                                                                                                                            |
