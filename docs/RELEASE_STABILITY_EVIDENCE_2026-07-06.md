...
This document records current release-stability evidence for the LoopRelay
...
| `corepack pnpm pack:dry-run`          | PASS   | Built server and web assets, ran the package dry-run wrapper, and produced `looprelay-1.0.0.tgz` with the expected shipped files, including this release evidence document.                                                                                                                                                                                                                                                                                                                |
| `corepack pnpm smoke:package-install` | PASS   | Built server and web assets, packed `looprelay-1.0.0.tgz`, installed it into an isolated npm prefix, verified the shipped `looprelay`, `lr-claude`, and `lr-codex` bin help paths from an isolated cwd, checked `looprelay start --open-web --json` returns the first-success setup, coach, and doctor commands, and verified the installed `looprelay quality-evidence --require-complete` release gate.                                                                               |
...
- `corepack pnpm --silent evidence:quality` reports `looprelay_95_quality`
  ...
  `looprelay quality-evidence --runtime-tool codex --require-runtime-ready`
