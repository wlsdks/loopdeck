...
사전등록한 real-task 기준을 충족했습니다. 교차 배정 10쌍, 5개 작업 유형,
유형별 2쌍입니다. 엄격 성공률은 baseline 0%, LoopRelay 30%였고 actionability는
49% 대 83%였습니다. LoopRelay는 평균 TTFV 7.2초, tool 3.3회, input token
95,945를 줄였습니다. 실패 3건이 개선됐고 7쌍은 fail/fail로 남았으며 human
review는 LoopRelay 8회, baseline 2회를 선호했습니다. 이는 maintainer-run
...
N=10 evidence 기반 scope:

- `retain`: Git에 없는 정확한 session/checkpoint 복구와 focused ambiguity 질문
- `narrow`: failure prevention은 다른 실제 사례의 strict success 전까지 제한
...
50%였고 그래프는 올바르게 `INSUFFICIENT DATA`였습니다. 현재 10쌍 결과는
...
fallback step을 삽입하지 못하게 명시합니다. 최종 집계는 10쌍·5개 유형,
엄격 성공률 baseline 0% 대 LoopRelay 30%입니다. Directional evidence이지만
