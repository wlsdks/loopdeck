...
describe("installLoopRelaySlashCommands", () => {
  it("copies every *.md file from sourceDir to <targetDir>/looprelay/", () => {
    ...
    const result = installLoopRelaySlashCommands({
      sourceDir,
      targetDir,
    });
    ...
    const namespaceDir = join(targetDir, "looprelay");
  ...
  it("rewrites files whose content drifted from the source (force = true default for looprelay namespace)", () => {
    const { sourceDir, targetDir } = makeFixture();
  ...
  it("dry-run reports the same shape but writes nothing", () => {
    ...
    expect(result.installed).toEqual(["guard.md"]);
    expect(() => readdirSync(join(targetDir, "looprelay"))).toThrow();
  });
  ...
  it("removes stale looprelay commands that no longer exist in the source", () => {
    const { sourceDir, targetDir } = makeFixture();
    seedSource(sourceDir, [["guard.md", "# guard\n"]]);
    const namespaceDir = join(targetDir, "looprelay");
    mkdirSync(namespaceDir, { recursive: true });
  ...
  it("ignores non-markdown files in the source dir", () => {
    ...
    expect(result.installed).toEqual(["guard.md"]);
    expect(readdirSync(join(targetDir, "looprelay"))).toEqual(["guard.md"]);
  });
});
...
function createTempDir(): string {
  const dir = join(tmpdir(), `looprelay-slash-${randomUUID()}`);
