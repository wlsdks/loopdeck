...
describe("installLoopSchedule", () => {
  it("dry-runs an explicit LaunchAgent for service-source loop collection without writing", () => {
    ...
    const plistPath = join(dir, "LaunchAgents", "com.looprelay.loop.plist");
    ...
    expect(result.nextPlist).toContain("com.looprelay.loop");
  ...
  it("writes the LaunchAgent only when explicitly not dry-run", () => {
    ...
    expect(plist).toContain("com.looprelay.loop");
...
function createTempDir(): string {
  const dir = join(tmpdir(), `looprelay-loop-schedule-${randomUUID()}`);
