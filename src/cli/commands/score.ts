...
function readScorePromptInput(options: ScoreCliOptions): string {
  ...
  if (raw === undefined) {
    throw new UserError(
      '--text or --stdin is required to score text directly. Try: looprelay score --text "add caching to fetchUser"',
    );
  }

  if (!raw.trim()) {
    throw new UserError(
      'Prompt must not be empty. Try: looprelay score --text "add caching to fetchUser"',
    );
  }
...
function formatArchiveScoreReport(report: ArchiveScoreReport): string {
  ...
  if (report.archive_score.scored_prompts === 0) {
    lines.push(
      "",
      "No prompts captured yet. Run looprelay start to see the shortest setup -> real prompt -> score path.",
    );
  }
