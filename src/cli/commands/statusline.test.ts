...
describe("renderClaudeCodeStatusLine", () => {
  it("renders only the score line when server, token, hook, and last ingest are all healthy", async () => {
    ...
    expect(line).toContain("run: /looprelay:improve-last");
...
describe("installClaudeCodeStatusLine", () => {
  ...
  it("uninstalls only looprelay statusLine entries", () => {
    const dir = createTempDir();
    const settingsPath = join(dir, "settings.json");
    installClaudeCodeStatusLine({ settingsPath });
...
describe("renderChainedClaudeCodeStatusLine", () => {
  it("prints looprelay on a separate line after an existing status line", () => {
    const line = renderChainedClaudeCodeStatusLine({
      previousCommand: "previous",
      loopRelayCommand: "looprelay",
      runCommand: (command) => ({
        stdout:
          command === "previous"
            ? "HUD ready\n"
  ...
  it("preserves multiline looprelay output after an existing status line", () => {
    const line = renderChainedClaudeCodeStatusLine({
      previousCommand: "previous",
      loopRelayCommand: "looprelay",
      runCommand: (command) => ({
  ...
  it("keeps looprelay output when the previous status line fails", () => {
    const line = renderChainedClaudeCodeStatusLine({
      previousCommand: "previous",
      loopRelayCommand: "looprelay",
      runCommand: (command) => ({
  ...
  it("passes Claude Code statusLine stdin to chained commands", () => {
    ...
    expect(calls).toEqual([
      ...
      { command: "looprelay", input: '{"cwd":"/Users/example/project"}' },
    ]);
  });
});

function createTempDir(): string {
  const dir = join(tmpdir(), `looprelay-statusline-${randomUUID()}`);
