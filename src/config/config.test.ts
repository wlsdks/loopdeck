...
describe("initializeLoopRelay", () => {
  ...
  it("is idempotent and does not rotate existing secrets", () => {
    ...
    const first = initializeLoopRelay({ dataDir });
    const second = initializeLoopRelay({ dataDir });
  ...
  it("defaults auto_judge to disabled on a fresh init", () => {
    const dataDir = createTempDir();
    const result = initializeLoopRelay({ dataDir });
  ...
  it("merges auto_judge patches without losing untouched fields", () => {
    ...
    const persisted = loadLoopRelayConfig(dataDir);
  ...
  it("defaults experimental_rules to an empty array on a fresh init", () => {
    ...
    expect(loadLoopRelayConfig(dataDir).experimental_rules).toEqual([]);
  });
  ...
  it("loads excluded_project_roots from config.json so server enforces them", () => {
    ...
    // Default
    expect(loadLoopRelayConfig(dataDir).excluded_project_roots).toEqual([]);
...
function createTempDir(): string {
  const dir = join(tmpdir(), `looprelay-config-${randomUUID()}`);
