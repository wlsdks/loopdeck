...
export const MCP_TOOL_CATALOG = [
  {
    ...
    when: "The user asks if looprelay is working, whether prompts are being captured, or what to do next.",
    returns:
    ...
    prompt:
      "Use looprelay get_looprelay_status and tell me whether capture is working before scoring anything.",
  },
  {
    kind: "coach",
    name: "coach_prompt",
    ...
    prompt:
      "Use looprelay coach_prompt and give me the latest score, first fix, recurring habit gap, and next request guidance.",
  },
  {
    kind: "single prompt",
    name: "score_prompt",
    ...
    prompt:
      "Use looprelay score_prompt with latest=true and tell me what to improve in my last request.",
  },
  {
    kind: "rewrite",
    name: "improve_prompt",
    ...
    prompt:
      "Use looprelay improve_prompt with latest=true and give me an approval-ready draft I can copy and resubmit.",
  },
  {
    ...
    privacy:
      "looprelay makes no external provider call; the redacted prompt is returned only to the active user-controlled agent session.",
    prompt:
      "Use looprelay prepare_agent_rewrite with latest=true. Rewrite that redacted prompt yourself, then ask before saving it.",
  },
  {
    kind: "advanced rewrite",
    ...
    prompt:
      "After I approve your rewrite, call looprelay record_agent_rewrite with provider, prompt_id, improved_prompt, confidence, summary, and changed_sections.",
  },
  {
    kind: "archive",
    ...
    prompt:
      "Use looprelay score_prompt_archive for recent Codex prompts and summarize my recurring prompt habit gaps.",
  },
  {
    kind: "project rules",
    ...
    prompt:
      "Use looprelay review_project_instructions with latest=true and tell me whether my AGENTS.md/CLAUDE.md rules are strong enough.",
  },
  {
    kind: "advanced judge",
    ...
    assurances: ["read-only", "opt-in", "redacted packet", "output schema"],
    privacy:
      "looprelay makes no external provider call; redacted prompt bodies are returned only for the active user-controlled agent session.",
    prompt:
      "Use looprelay prepare_agent_judge_batch with selection=low_score and max_prompts=5. Judge those redacted prompts yourself.",
  },
  {
    kind: "advanced judge",
    ...
    prompt:
      "After judging the prepared packet, call looprelay record_agent_judgments with one score, confidence, summary, risks, and suggestions per prompt_id.",
  },
];
...
export function createMcpReadiness({
  ...
  let firstCall = "get_looprelay_status";
  let nextAction =
