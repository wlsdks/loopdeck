export const HOOK_COPY = {
  en: {
    contextHeader: "LoopRelay rewrite guidance",
    contextHint:
    ...
    blockedReason: (score: number, band: string, minScore: number) =>
      `looprelay blocked this prompt before submission because its local score was ${score}/100 (${band}), below the configured threshold ${minScore}.`,
    clipboardHit:
  ...
  ko: {
    contextHeader: "LoopRelay 개선안 가이드",
    contextHint:
      "원래 프롬프트보다 명확한 경우 아래 개선안을 작업 brief로 사용해주세요.",
    scoreLine: (score: number, band: string) =>
      `원래 로컬 점수: ${score}/100 (${band})`,
    blockedReason: (score: number, band: string, minScore: number) =>
      `looprelay가 이 프롬프트를 제출 전 차단했습니다. 로컬 점수 ${score}/100 (${band})가 설정된 기준 ${minScore} 미만입니다.`,
    clipboardHit:
