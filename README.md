...
Current results are maintainer-run observational evidence, not a causal claim. They include 21 matched pairs across 5 task types and 0/3 independent users. A separate cohort has 3 independent agent operators with 100% first-value success; agent operators do not count as human users.

| Task type | Pairs | Baseline success | LoopRelay success | Delta | Conservative 95% bound | Input-token delta | Decision |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| Ambiguity clarification | 3 | 100% | 66.7% | -33.3pp | -100..100pp | +5343.7 | Collect more |
...
| Release verification continuity | 3 | 100% | 100% | 0pp | -100..100pp | +19112 | Collect more |
| Session recovery | 5 | 20% | 80% | +60pp | -61.5..100pp | -3240.4 | Retain |

Aggregate success moved from 57.1% to 85.7%, while actionability moved from 71.9% to 90%. Mean input-token cost changed by 18.7%. Cached-token and TTFV condition coverage are 52.4% and 52.4% respectively; missing values are not interpreted as zero. Only 3/5 target task types currently meet the per-type minimum of 5 pairs, so every task-specific decision remains provisional until coverage is complete. Because ordinary implementation continuation regressed, LoopRelay should not intervene by default in every coding task. The causal claim remains false until independent-user validation is complete.
