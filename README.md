...
Current results are maintainer-run observational evidence, not a causal claim. They include 17 matched pairs across 5 task types and 0/3 independent users. A separate cohort has 3 independent agent operators with 100% first-value success; agent operators do not count as human users.
...
Aggregate success moved from 47.1% to 88.2%, while actionability moved from 65.3% to 91.2%. Mean input-token cost changed by 23%. Cached-token and TTFV condition coverage are 41.2% and 41.2% respectively; missing values are not interpreted as zero. Only 3/5 target task types currently meet the per-type minimum of 5 pairs, so every task-specific decision remains provisional until coverage is complete. Because ordinary implementation continuation regressed, LoopRelay should not intervene by default in every coding task. The causal claim remains false until independent-user validation is complete.
